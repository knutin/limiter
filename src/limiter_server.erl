...
-export([cancel_timer/1, set_ts/2, force_plan/1, get_tokens/1, get_capacity/1]).
...
force_plan(Limiter) -> gen_server:call(Limiter, force_plan).
...
handle_call({do, Request, WaitStart, {WaitTimeout, _} = Timeouts},
    ...
    case enough_time(WaitStart, WaitTimeout, State#state.predicted_response_time) of
        true ->
            error_logger:info_msg("in: ~p~n", [Request]),
...
handle_call(force_plan, _From, State) ->
    {reply, ok, spawn_workers(plan(State))};
            ...
            case Response of
                {ok, _} ->
                    ...
                    {noreply, State#state{workers = NewWorkers, num_workers = N-1,
                                          tokens = [{State#state.current_ts,
                                                     max(Tokens-UsedUnits, 0)} | OldTokens]
                                         }};
                {error, overload} ->
...
plan(State) ->
    ...
    error_logger:info_msg("planning, t=~p, prev=~p~n"
                          "overload_history=~p~n"
                          "estimated_capacity=~p~n",
                          [T, PrevT, State#state.overload_history,
                           State#state.estimated_capacity]),

    case proplists:get_value(PrevT, State#state.overload_history, 0) of
        0 ->
            NewEstimatedCapacity = PrevEstimatedCapacity * 1.1,
            error_logger:info_msg("T: ~p, new estimate: ~p~n", [T, NewEstimatedCapacity]),
            State#state{tokens = [{T, trunc(NewEstimatedCapacity)} | State#state.tokens],
                        estimated_capacity = [{T, trunc(NewEstimatedCapacity)} |
                                              State#state.estimated_capacity],
                        overload_history = [{T, 0} | State#state.overload_history],
                        pid = limiter_pid_controller:clear(State#state.pid, T)};
        Errors ->
            {NewEstimatedCapacity, NewPid} =
                limiter_pid_controller:update(State#state.pid, T, Errors),

            error_logger:info_msg("errors=~p prev_estimate=~p new=~p~n",
                                  [Errors, PrevEstimatedCapacity, NewEstimatedCapacity]),
            State#state{tokens = [{T, trunc(NewEstimatedCapacity)} | State#state.tokens],
                        estimated_capacity = [{T, trunc(NewEstimatedCapacity)} |
                                              State#state.estimated_capacity],
                        overload_history = [{T, 0} | State#state.overload_history],
